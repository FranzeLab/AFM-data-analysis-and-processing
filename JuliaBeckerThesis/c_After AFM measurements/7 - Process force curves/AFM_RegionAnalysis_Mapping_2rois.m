%% AFM_RegionAnalysis_Mapping_2rois.m          by Julia Becker, 14/10/2021
% This script is identical to AFM_RegionAnalysis_Mapping.m other than that
% it allows the user to define two unrelated sets of ROIs. The 'data'
% variable will contain two columns ('roi' and 'roi2'), one for each set of
% annotated ROIs. This can be useful if ROIs are overlapping as the ROI
% annotation does not allow points to belong to more than one layer. Adding
% a second set of ROIs can solve this.

% Features identical to AFM_RegionAnalysis_Mapping.m:
...
%   - Heatmap plotting:
%         - 'markersize', 'bar_intervals', 'colourmap' - see comments there
%         - answers to questions being asked before plotting - see there
%
% BEFORE USING THIS SCRIPT:
% - Have your data ordered in Julia's standard folder setup (ask if needed)
% - Run batchforce.m
% - Sort your force curves (AssessFits_batchforce_SortCurves.m)
% (- no WriteLabFile.m or similar required any longer)
%
% OUTPUT:
...
%% Second region analysis, if requested
ShowImage(I)
ui2b = questdlg('Do you want to do an independent second region analysis?', 'Second region analysis', 'Yes', 'No', 'Yes');

if strcmp(ui2b, 'No') == 1
    if ismember('roi2',data.Properties.VariableNames) == 1
        if length(unique(data.roi2)) > 1
            ui3b = questdlg('Regions are already annotated for this dataset. Do you want to remove these?', 'Overwrite existing data?', 'Keep', 'Remove', 'Keep');
            if strcmp(ui3b, 'Remove') == 1
                data.roi2(:) = 0;
                layer_names2 = string([]);
                layer_numbers2 = size(layer_names2,1);
            end
        else
            data.roi2(:) = 0;
            layer_names2 = string([]);
...
elseif strcmp(ui2b, 'Yes') == 1
    if ismember('roi2',data.Properties.VariableNames) == 0
        roi2 = zeros(height(data),1);
        data = [data table(roi2)];
        clear roi2
   %Comment the next two lines in in order to remove previous annotations
   %else
       %data.roi2(:) = 0;
    end
               
    %% Option to predefine layer_names and layer_number - refer to comments if unwanted
    layer_names2 = string([]);                      % to predefine: e.g. layer_names = string(["WhiteMatter"; "GreyMatter"]);    to manually define later: layer_names = string([]); 
    layer_numbers2 = [];                            % if layer_names predefined: layer_numbers = size(layer_names,1);            to manually define later: layer_numbers = [];
 
    %%
    ui3b = questdlg('Which type of region analysis do you want to do?', 'Region analysis', 'Standard', 'Unbiased', 'Standard');
    if strcmp(ui3b, 'Standard') == 1
        [data, layer_numbers2, layer_names2, colours2, colour_scheme2] = RegionAnalysis_Standard_2rois(data, I, source, layer_numbers2, layer_names2);
        save(source, 'data', 'ui3b', 'layer_numbers2', 'layer_names2', 'colours2', 'colour_scheme2', '-append')
    elseif strcmp(ui3b, 'Unbiased') == 1
        [data, annotations2, layer_numbers2, layer_names2, colours2, colour_scheme2] = RegionAnalysis_Unbiased_2rois(data, I, source, layer_numbers2, layer_names2);
        save(source, 'data', 'ui3b', 'layer_numbers2', 'layer_names2', 'colours2', 'colour_scheme2', 'annotations2', '-append')
    end
    uiwait(msgbox("Image will close when you click 'Okay'."))
end
close all
...
bar_intervals = 150;                        % interval for ticks on colourbar
colourmap = 'jet';                          % for all default colourmaps, see https://uk.mathworks.com/help/matlab/ref/colormap.html --> Input arguments --> map
