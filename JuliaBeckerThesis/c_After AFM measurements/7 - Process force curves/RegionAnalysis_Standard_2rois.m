...
if (or(isempty(layer_numbers), isempty(layer_names)) == 1)
    layer_numbers = inputdlg('How many different layers do you want to analyse? (max 7)');          % if >6 layers needed, will need to implement more colours - just change variable 'colours'!
...
colours = {'red','r';'cyan','c';'green','g';'yellow','y';'magenta','m'; 'red','r';'cyan','c';'green','g';'yellow','y';'magenta','m';};

% Display image with inactivated points and instruct user
hold on
plot(data.x_image(find(data.roi2 == 0)), data.y_image(find(data.roi2 == 0)), '.', 'MarkerSize', 8, 'LineWidth', 2, 'Color', 'b');
for i = 1:layer_numbers
    plot(data.x_image(find(data.roi2 == i)), data.y_image(find(data.roi2 == i)), '.', 'MarkerSize', 8, 'LineWidth', 2, 'Color', colours{i,2});
end
    ...
    while 1
        ...
        for k = 1:size(data,1)
            if mask{i,1}(round(data.y_image(k)),round(data.x_image(k))) == 1             
                % Assign new roi2 values according to mask  - move this point one layer down
                current_value = data.roi2(k);
                new_value = order((find(order == current_value,1))+1);
                data.roi2(k) = new_value;
            end
        end
        
        % Replot all points according to their roi2 value
        plot(data.x_image(find(data.roi2 == 0)), data.y_image(find(data.roi2 == 0)), '.', 'MarkerSize', 8, 'LineWidth', 2, 'Color', 'b');
        for i = 1:layer_numbers
            plot(data.x_image(find(data.roi2 == i)), data.y_image(find(data.roi2 == i)), '.', 'MarkerSize', 8, 'LineWidth', 2, 'Color', colours{i,2});
        end
...
figurewithlayers_name = fullfile(source(1:end-9),'annotated layers_2.tif');
...
fileID = fopen(fullfile(source(1:end-9),'annotated layers_2_colour scheme.txt'),'w');
formatSpec = '%s\t%s\n';
